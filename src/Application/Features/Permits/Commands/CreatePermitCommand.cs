...
public class CreatePermitCommand : IRequest<Either<PermitException, Permit>>,
    IValidatableModel<CreatePermitCommand>
{
    ...
    public required IReadOnlyList<EmissionLimitCommandDto> EmissionLimits { get; init; }

    public IValidator<CreatePermitCommand> ValidateApplicationModel(
        ApplicationBaseValidationModelProvider<CreatePermitCommand> validator)
    {
        validator.RuleFor(x => x.InstallationId).NotEmpty();
        ...
        validator.RuleForEach(x => x.EmissionLimits)
            .SetValidator(new EmissionLimitDtoValidator());

        return validator;
    }

    private bool NoOverlappingLimits(IReadOnlyList<EmissionLimitCommandDto> limits)
    {
